...
function saveCurrentMessage(silent = false) {
    ...
    try {
        ...
        const messages = JSON.parse(localStorage.getItem('morse_messages') || '[]');
        ...
        localStorage.setItem('morse_messages', JSON.stringify(messages));
        ...
        if (!silent) {
            showToast('Failed to save message', 'error', 3000);
        }
    }
}
...
async function translateCurrentMessage() {
    ...
    try {
        showToast('Translating...', 'info', 1000);
        ...
        if (result.translatedText) {
            const confirmed = confirm(`Translated to ${targetLang.toUpperCase()}:\n\n${result.translatedText}\n\nSave translated version?`);
...
function getSettings() {
    try {
        return JSON.parse(localStorage.getItem('morse_settings') || '{}');
    } catch {
        return {};
    }
}
